...
try {
    ...
    $stats['total_alertes'] = $stmt->fetchColumn();
    
    // Alertes par statut
    $stmt = $pdo->query('SELECT statut, COUNT(*) as count FROM signalements GROUP BY statut ORDER BY FIELD(statut, "nouveau", "en_cours", "traite", "cloture", "rejete")');
    $stats['par_statut'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Alertes par type (top 5)
    $stmt = $pdo->query('SELECT t.nom, COUNT(s.id) as count 
                        FROM signalements s 
    ...
    // Dernières alertes
    $stmt = $pdo->query('SELECT s.id, s.date_creation, s.statut, s.niveau_gravite, t.nom as type_alerte
                         FROM signalements s
                         LEFT JOIN types_alerte t ON s.type_alerte_id = t.id
                         ORDER BY s.date_creation DESC
                         LIMIT 8');
    $stats['dernieres_alertes'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Temps de traitement moyen
    $stmt = $pdo->query('SELECT 
        AVG(TIMESTAMPDIFF(HOUR, date_creation, date_maj)) as temps_moyen_heures
        FROM signalements 
        WHERE statut IN ("traite", "cloture") AND date_maj IS NOT NULL');
    $stats['temps_traitement_moyen'] = round($stmt->fetchColumn(), 1);
    
    // Gravité moyenne
    $stmt = $pdo->query('SELECT AVG(niveau_gravite) as gravite_moyenne FROM signalements WHERE niveau_gravite IS NOT NULL');
    $stats['gravite_moyenne'] = round($stmt->fetchColumn(), 1);
    
    // Alertes aujourd'hui
    $stmt = $pdo->query('SELECT COUNT(*) as aujourdhui FROM signalements WHERE DATE(date_creation) = CURDATE()');
    $stats['aujourdhui'] = $stmt->fetchColumn();
    
    // Alertes cette semaine
    $stmt = $pdo->query('SELECT COUNT(*) as cette_semaine FROM signalements WHERE YEARWEEK(date_creation) = YEARWEEK(CURDATE())');
    ...
    $stats['taux_traitement'] = $stats['total_alertes'] > 0 ? round(($total_traitees / $stats['total_alertes']) * 100, 1) : 0;
        ...
        <header class="header">
            <div class="header-left">
                <div>
                    ...
                    <p class="header-subtitle">Vue d'ensemble des signalements</p>
                </div>
            </div>
    ...
    <script>
        ...
        document.querySelectorAll('.chart-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Ici vous pourriez ajouter la logique pour changer les données du graphique
